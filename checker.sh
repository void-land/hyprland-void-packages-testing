...
main() {
	...
	# Find only Hyprland-related folders, excluding *-devel
	for package_dir in $(find "$template_dir" -mindepth 1 -maxdepth 1 -type d -name 'hypr*' ! -name '*-devel'); do
		pkgname=$(basename "$package_dir")
		template_file="${package_dir}/template"

		if [[ ! -f "$template_file" ]]; then
			log warning "Template file not found for $pkgname"
			continue
		fi
		...
		if [[ -z "$latest_version" ]]; then
			...
			log success "$pkgname is up-to-date (Version: $current_version)"
		else
